# 🏆 Phase 3: Advanced Competition & Leadership
> **Prerequisite:** Veteran status, Subteam Lead role, or Mentor approval.

Building the robot is only half the battle. Phase 3 is reserved for veteran members, subteam leads, and the drive team. 

In this phase, you will zoom out from individual part creation to focus on macro-level team operations and event execution. This is where you learn how to read a game manual to find strategic loopholes, manage a high-stress pit crew during an 8-minute turnaround, and lead engineering design reviews. 

!!! info "Stepping into Leadership"
    Unlike Phases 1 and 2, Phase 3 modules are not strictly linear. You may be asked by a mentor to complete a specific module based on your leadership role for the season (e.g., the Scouting Lead will focus heavily on Pathway 3.1, while the Pit Boss will focus on 3.2). 

---

## 🗺️ Select Your Leadership Pathway

<div class="grid cards" markdown>

-   <span style="color: var(--md-primary-fg-color);">:material-poll:</span> **Pathway 3.1: Strategy & Data Analytics**

    ---

    *Take the game from the shop to the field.* Master the Game Manual, design scouting architectures, and learn alliance selection theory.
    
    [Start Strategy Pathway](../strategy/game-manual.md){ .md-button .md-button--primary }

-   <span style="color: var(--md-primary-fg-color);">:material-car-pitstop:</span> **Pathway 3.2: Pit Crew & Event Ops**

    ---

    *Execute under pressure.* Train for the 8-minute turnaround, manage the packing manifest, and navigate the robot inspection process.
    
    [Start Pit Crew Pathway](../competition/packing.md){ .md-button .md-button--primary }

-   <span style="color: var(--md-primary-fg-color);">:material-clipboard-text-outline:</span> **Pathway 3.3: Project Management**

    ---

    *Run the team like a firm.* Required for all Captains and Leads. Master the 6-week timeline, budgeting, and the art of mentorship.
    
    [Start Leadership Pathway](../leadership/timeline.md){ .md-button .md-button--primary }

</div>

---

!!! quote "Core Value Check"
    *"A good engineer builds a great robot. A great engineer builds a great team."*
